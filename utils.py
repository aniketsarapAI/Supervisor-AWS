...
class ActiveFilters(BaseModel):
    ...
    Replaces untyped dict approach.
    """
    framework: str | None = Field(default=None, description="Framework context (e.g., FastAPI, LangGraph, Docker)")
    library: str | None = Field(default=None, description="Library context (e.g., langchain, pydantic)")
    language: str | None = Field(default=None, description="Programming language context (e.g., Python, TypeScript)")
...
class AgentState(BaseModel):
    ...
    query_analysis: QueryAnalysis | None = Field(default=None, description="Structured query analysis output from the Query Analysis node.")
    active_filters: ActiveFilters = Field(default_factory=ActiveFilters, description="Working copy of active filters from Conversation Module.")
...
# NODE: QUERY ANALYSIS
def query_analysis_node(state: AgentState) -> Command[Literal["__end__"]]:
    """
    Structured Query Analysis node.
    Replaces the old supervisor classification approach.
    Uses enriched context from the Conversation Module:
    current query + recent messages + active filters.
    """
    pyd_parser = PydanticOutputParser(pydantic_object=QueryAnalysis)

    prompt_template = PromptTemplate(
        template="""
You are a Query Analysis agent responsible for analyzing user queries and producing structured output for deterministic routing.

Analyze the conversation context and determine:
1. The resolved query (self-contained, considering conversation history)
2. The intent: conversation, knowledge, coding, or escalation
3. Extracted entities that could become active filters (framework, library, language, topic)
4. Confidence in your classification (0.0 to 1.0)
5. Whether human intervention is required
6. Whether clarification is needed (query too vague/broad)
7. A clarification question if needed
8. Whether the topic has changed from previous context

Intent definitions:
  - conversation: greetings, thanks, goodbye, casual chat, meta-questions about the assistant
  - knowledge: factual questions, documentation lookup, how-to questions, explanations
  - coding: code generation, debugging, code review, implementation tasks
  - escalation: user is frustrated, repeated failures, explicit request for human help
...
""",
    ...
    chain = prompt_template | llm_model | pyd_parser
